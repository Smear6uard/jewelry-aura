...
import { serverFn, withInput } from './server-fn'
...
// ---------------------------------------------------------------------------
// Server functions (thin wrappers; all builder wiring goes through server-fn.ts)
// ---------------------------------------------------------------------------

/** Current cart, or null when there is no cookie or the cart is gone. GET —
 * never writes cookies, so responses stay CDN-cacheable (KTD3). */
export const getCart = serverFn('GET').handler(() => getCartLogic())

export const addToCart = withInput(
  serverFn('POST'),
  validateAddToCartInput,
).handler(({ data }) => addToCartLogic(data))

export const updateCartLine = withInput(
  serverFn('POST'),
  validateUpdateCartLineInput,
).handler(({ data }) => updateCartLineLogic(data))

export const removeCartLine = withInput(
  serverFn('POST'),
  validateRemoveCartLineInput,
).handler(({ data }) => removeCartLineLogic(data))
